...
class _AdminContentScreenState extends State<AdminContentScreen> {
  ...
  String _selectedSubject = kTeachableSubjects.first.title;
  ...
  Future<void> _initSubjectData() async {
    try {
      ...
      if (subjSnap.exists && subjSnap.data() != null) {
        ...
        final schoolSubject = kTeachableSubjects.firstWhere(
            (s) => s.title == _selectedSubject,
            orElse: () => kTeachableSubjects.first);
        if (unitsRaw is List) {
  ...
  Future<void> _confirmCleanupGrades(BuildContext ctx) async {
    final confirm = await showDialog<bool>(
      context: ctx,
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: Text('تأكيد التنظيف والدمج', style: GoogleFonts.tajawal(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    if (ctx.mounted) {
      ScaffoldMessenger.of(ctx).showSnackBar(
        SnackBar(
          content: Text('جاري تنظيف ودمج قاعدة البيانات... (قد يستغرق بعض الوقت)', style: GoogleFonts.tajawal()),
          duration: const Duration(seconds: 3),
        ),
      );
    }

    final deletedCount = await DatabaseCleanupService.deleteUnusedGradesData();
    await DatabaseCleanupService.migrateOldSubjectsToNewFormat();
        ...
        actions: [
          if (isSuperAdmin)
            IconButton(
              ...
              tooltip: 'حذف بيانات الصفوف غير المستخدمة من السحابة',
              onPressed: () => _confirmCleanupGrades(context),
            ),
          if (isSuperAdmin)
            IconButton(
